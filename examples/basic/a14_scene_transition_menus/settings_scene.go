...
func newSettingsScene(name string, atlas api.IAtlasX, world api.IWorld) (api.INode, error) {
	o := new(settingsMenu)
	o.Initialize(name)
	o.atlas = atlas
...
func (s *settingsMenu) build(world api.IWorld) error {
	s.Node.Build(world)
	...
	newSettingsLayer("Menu Layer", s.atlas, world, s)
