...
func newMenuScene(name string, atlas api.IAtlasX, world api.IWorld) (api.INode, error) {
	o := new(sceneMenu)
	o.Initialize(name)
	o.atlas = atlas
...
func (s *sceneMenu) build(world api.IWorld) error {
	s.Node.Build(world)
	...
	newMenuLayer("Menu Layer", s.atlas, world, s)
