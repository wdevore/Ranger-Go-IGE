...
func newSettingsScene(name string, world api.IWorld, fontRenderer api.ITextureRenderer) (api.INode, error) {
	o := new(settingsMenu)
	o.Initialize(name)
...
func (s *settingsMenu) build(world api.IWorld, fontRenderer api.ITextureRenderer) error {
	s.Node.Build(world)
	...
	newSettingsLayer("Menu Layer", world, fontRenderer, s)

	return nil
}

func (s *settingsMenu) Update(msPerUpdate, secPerUpdate float64) {
...
func (s *settingsMenu) setState(header string, state int) {
	s.SetCurrentState(state)
	// nodes.ShowState(header, s, "")
}

func (s *settingsMenu) Notify(state int) {
...
func (s *settingsMenu) EnterScene(man api.INodeManager) {
	// fmt.Println("settingsMenu EnterScene")
	vrs := s.World().Properties().Window.DeviceRes
...
func (s *settingsMenu) ExitScene(man api.INodeManager) bool {
	// fmt.Println("settingsMenu ExitScene")
...
func (s *settingsMenu) Handle(event api.IEvent) bool {
