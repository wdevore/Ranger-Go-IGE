...
func newHighscoreScene(name string, world api.IWorld, fontRenderer api.ITextureRenderer) (api.INode, error) {
	o := new(highscoreMenu)
	o.Initialize(name)
...
func (s *highscoreMenu) build(world api.IWorld, fontRenderer api.ITextureRenderer) error {
	s.Node.Build(world)

	dvr := s.World().Properties().Window.DeviceRes

	bg := newBackgroundNode("Background", world, s, color.NewPaletteInt64(color.Peach))
	bg.SetScaleComps(float32(dvr.Width), float32(dvr.Height))

	newHighscoreLayer("Highscore Layer", world, fontRenderer, s)

	return nil
}

func (s *highscoreMenu) Update(msPerUpdate, secPerUpdate float64) {
...
func (s *highscoreMenu) setState(header string, state int) {
	s.SetCurrentState(state)
	// nodes.ShowState(header, s, "")
}

func (s *highscoreMenu) Notify(state int) {
...
func (s *highscoreMenu) EnterScene(man api.INodeManager) {
	// fmt.Println("highscoreMenu EnterNode")
	vrs := s.World().Properties().Window.DeviceRes
...
func (s *highscoreMenu) ExitScene(man api.INodeManager) bool {
	// fmt.Println("highscoreMenu ExitNode")
...
func (s *highscoreMenu) Handle(event api.IEvent) bool {
