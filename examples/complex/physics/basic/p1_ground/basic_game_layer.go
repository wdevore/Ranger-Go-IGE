...
func (g *gameLayer) addGround() error {
	...
	g.groundLineNode, err = shapes.NewMonoHLineNode("Ground", g.World(), g)
	if err != nil {
		return err
	}
	...
	ghl := g.groundLineNode.(*shapes.MonoHLineNode)
...
func (g *gameLayer) addCircle() error {
	...
	g.fallingCirNode, err = shapes.NewMonoCircleNode("Circle", api.FILLED, 12, g.World(), g)
	if err != nil {
		return err
	}
	g.fallingCirNode.SetScale(3.0)
	g.fallingCirNode.SetPosition(fallingCirPos.X(), fallingCirPos.Y())
	gc := g.fallingCirNode.(*shapes.MonoCircleNode)
...
func (g *gameLayer) addSquare() error {
	...
	g.fallingSqrNode, err = shapes.NewMonoSquareNode("Square", api.FILLED, true, g.World(), g)
	if err != nil {
		return err
	}
	g.fallingSqrNode.SetScale(3.0)
	g.fallingSqrNode.SetPosition(fallingSqrPos.X(), fallingSqrPos.Y())
	gsq := g.fallingSqrNode.(*shapes.MonoSquareNode)
...
func (g *gameLayer) addTri() error {
	...
	g.fallingTriNode, err = shapes.NewMonoTriangleNode("Triangle", api.FILLED, g.World(), g)
	if err != nil {
		return err
	}
	g.fallingTriNode.SetScale(3.0)
	g.fallingTriNode.SetPosition(fallingTriPos.X(), fallingTriPos.Y())
	gt := g.fallingTriNode.(*shapes.MonoTriangleNode)
...
func buildGroundPhysics(g *gameLayer) {
	...
	tln := g.groundLineNode.(*shapes.MonoHLineNode)
