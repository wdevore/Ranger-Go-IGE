...
// NewCircleNode constructs a rectangle shaped node
...
func (r *CircleNode) Build(world api.IWorld) error {
	r.Node.Build(world)

	r.color = rendering.NewPaletteInt64(rendering.LightPurple).Array()

	r.shape = world.Atlas().Shape("Circle12Segments")

	return nil
}

// SetColor sets line color
func (r *CircleNode) SetColor(color api.IPalette) {
	r.color = color.Array()
}

// Draw renders shape
func (r *CircleNode) Draw(model api.IMatrix4) {
	// if r.IsDirty() {
	// 	r.SetDirty(false)
	// }
