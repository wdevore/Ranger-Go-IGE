...
func (r *rasterFont) Initialize(dataFile string, relativePath string) error {
	dataPath, err := filepath.Abs(relativePath)
	if err != nil {
		fmt.Println("RasterFont: relativePath error:")
		return err
	}

	fontFile := dataPath + "/assets/" + dataFile
	file, err := os.Open(fontFile)
	defer file.Close()

	if err != nil {
		fmt.Println("RasterFont: Opening ", fontFile)
		return err
	}
