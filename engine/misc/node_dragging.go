...
func (d *dragState) SetMotionState(x, y int32, state uint32) {
	if d.active {
		if d.dragging {
			d.delta.SetByComp(float32(x)-d.position.X(), float32(y)-d.position.Y())
			d.position.SetByComp(float32(x), float32(y))
		}
	}
}
...
func (d *dragState) SetButtonState(x, y int32, button uint8, state uint32) {
	...
	if d.active {
		d.positionDown.SetByComp(float32(x), float32(y))
		d.position.SetByPoint(d.positionDown)
	} else {
		d.positionUp.SetByComp(float32(x), float32(y))
	}
}
