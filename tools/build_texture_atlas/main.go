...
const (
	// Outputs: two files
	textureAtlasFile = "starship_texture_atlas.png"
	textureManifest  = "texture_manifest.json"
	// Input
...
func main() {
	...
	// writeAsFlatFile(textureManifest, textureAtlasFile, packSize, packSize, blocks)
	writeAsJSONFile(textureManifest, textureAtlasFile, packSize, packSize, blocks)
}
...
func collectFiles(path string) []string {
	...
	fmt.Println("Collecting files from: ", path)

	if strings.Contains(dataPath+path, "*") {
